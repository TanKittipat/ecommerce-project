...
const Service = () => {
  const [myServices, setMyServices] = useState(serviceLists);
  return (
            ...
            {myServices.length > 0 &&
              myServices.map((service) => {
                return (
                  <div
                    key={service.id}
                    className="shadow-md h-52 rounded-sm py-5 px-4 text-center space-y-2 text-red cursor-pointer hover:border hover:border-red transition-all duration-300"
                  >
                    <img
                      src={service.image}
                      alt={service.title}
                    ...
                    <p className="text-[#907E7E]">{service.description}</p>
                  </div>
                );
              })}
          </div>
